...
function surveyTimer(delay) {
    setTimeout(() => {
        console.log('Survey results', oldestValid);
    }, delay);
}
...
setTimeout(() => {
    ...
    pupCoin.createTransaction(new Transaction(null, 'address1', 50));
    broadcast({type: 'USER_PRGM_CHAIN', address: MY_ADDRESS, pupCoin});
    setTimeout(() => {
        console.log('Getting oldest valid');
        broadcast({ type: 'SURVEY', address: MY_ADDRESS });
        surveyTimer(5000);
    }, 5000);
}, 5000);